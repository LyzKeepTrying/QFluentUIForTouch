...
void FluentSpinBox::paintEvent(QPaintEvent* event) {
    ...
    // 2. 计算按钮大小和位置（正方形，边长 = 高度）
    int btnSize = height();
    minus_button_rect_ = QRect(0, 0, btnSize, btnSize).adjusted(4, 4, -4, -4);
    plus_button_rect_ = QRect(width() - btnSize, 0, btnSize, btnSize).adjusted(4, 4, -4, -4);

    // 3. 准备绘制按钮内部样式（可根据状态改颜色，这里用背景色举例）
    painter.setPen(Qt::NoPen);
    painter.setBrush(getMinusButtonIsPressed() ? getBackgroundColor().darker(160) : getBackgroundColor().darker(110)); // 稍微深一点的按钮底色
    painter.drawRoundedRect(minus_button_rect_, 8, 8);
    painter.setBrush(getPlusButtonIsPressed() ? getBackgroundColor().darker(160) : getBackgroundColor().darker(110)); // 稍微深一点的按钮底色
    painter.drawRoundedRect(plus_button_rect_, 8, 8);

    // 4. 在按钮里画“–”和“+”
    painter.setPen(getTextColor());
    QFont text_font = QFluentUI::Font::default_text_font;
    text_font.setPixelSize(btnSize / 1.5);
    ...
    painter.setFont(text_font);
    QRect text_rect = rect().adjusted(btnSize + 8, 0, -btnSize - 8, 0);
    painter.setPen(getTextColor());
    painter.setFont(text_font);
